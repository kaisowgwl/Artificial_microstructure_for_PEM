        ...
        while phase_volume_filled(current_phase,1) <= phase_volume_target && stop_condition_not_reached
            ...
            if rn < (remaining_volume_fraction_currentphase_z+tolerance_vf)/normalized_term % Check volume fraction
                ...
                if current_diameter==1 || current_diameter==2  % One-voxel particle
                    ...
                    dy = dx * (1/current_elongation_dx_over_dy); % Particle lenght along axe 2
                    dz = dx * (1/current_elongation_dx_over_dz); % Particle lenght along axe 3
              %      fprintf('Calling create_ellipsoid with dx=%d, dy=%d, dz=%d\n', dx, dy, dz);
binary_ellipsoid = create_ellipsoid(dx, dy, dz);
size_ellipsoid = size(binary_ellipsoid);
%fprintf('Size of binary_ellipsoid after creation: [%d %d %d]\n', size_ellipsoid(1), size_ellipsoid(2), size_ellipsoid(3));
...
% Make sure this line accurately captures the dimensions
size_ellipsoid = size(binary_ellipsoid);
%fprintf('Size of binary_ellipsoid directly after creation: [%d %d %d]\n', size_ellipsoid(1), size_ellipsoid(2), size_ellipsoid(3));
...
% if z_center < 1 || z_center > size(phase(current_phase).volumefraction.along_3rd_axis_allslices, 1)
%     error(['z_center (', num2str(z_center), ') is out of bounds for volumefraction array size ', num2str(size(phase(current_phase).volumefraction.along_3rd_axis_allslices, 1))]);
% end

% For example, checking accesses to z_center
% if z_center > size(phase(current_phase).volumefraction.along_3rd_axis_allslices, 1)
%     error('z_center index exceeds the size of volumefraction array.');
% end
% Safely access the volumefraction array with checks
% if z_center <= size(phase(current_phase).volumefraction.along_3rd_axis_allslices, 1)
%     remaining_volume_fraction_currentphase_z = max(0, phase(current_phase).volumefraction.along_3rd_axis_allslices(z_center) - phase(current_phase).current_volumefraction.along_3rd_axis_allslices(z_center));
% else
...
% Assume size_ellipsoid, x_center, y_center, z_center, and domain_size are already defined



% Ensure that z_min and z_max are within the valid range of the 3D array dimensions


% Now z_min and z_max are safe to use for array indexing
subdomain = microstructure3D.phase(:,:,z_min:z_max); % Example of using z_min and z_max safely
...
function [binary_ellipsoid] = create_ellipsoid(dx, dy, dz)
    % Round up to avoid sub-voxel dimensions
    dx = max(ceil(dx), 2);
    dy = max(ceil(dy), 2);
    dz = max(ceil(dz), 2);

    % Define radii based on adjusted dimensions
    Rx = dx / 2;
    Ry = dy / 2;
    Rz = dz / 2;
    ...
    % Generate binary ellipsoid
    binary_ellipsoid = zeros(size(X));
    binary_ellipsoid(R <= 1) = 1;
end
